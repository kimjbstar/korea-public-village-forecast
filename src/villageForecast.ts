...
export class VillageForecast {
    ...
    // 1. 초단기실황조회
    async getUltraSrtNcst(lat: number, lng: number, datetime?, numOfRows = 1024, pageNo = 1): Promise<NCastResult> {
        ...
        const result = body.items.item.reduce((result, item) => {
            if (!result.baseDate) {
                result.baseDate = moment(`${item.baseDate} ${item.baseTime}`).format('YYYY-MM-DD HH:mm')
            }
    ...
    // 2.초단기예보
    async getUltraSrtFcst(lat: number, lng: number, datetime?, numOfRows = 1024, pageNo = 1): Promise<FCastResult> {
        let baseDatetime: moment.Moment = null;
        if (!datetime) {
            baseDatetime = moment();
        ...
        result.forecastDate = Object.entries(result.forecastDate).reduce((result, [k, v]) => {
            const newRow = {
    ...
    // 3. 동네예보조회
    async getVilageFcst(lat: number, lng: number, datetime?, numOfRows = 1024, pageNo = 1): Promise<FCastResult> {
        let baseDatetime: moment.Moment = null;
        if (!datetime) {
            baseDatetime = moment();
            // TODO
    ...
    // 4. 동네예보조회
    async getFcstVersion(type: 'ODAM' | 'VSRT' | 'SHRT', numOfRows = 1024, pageNo = 1) {
        const params = {
            serviceKey: this.serviceKey,
            pageNo: pageNo,
            numOfRows: numOfRows,
